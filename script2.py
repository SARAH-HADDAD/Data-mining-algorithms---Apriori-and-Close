...
for min_confidence in min_confidence_range:
    # Determine the minimum support using the 50/50 rule
    num_transactions = len(data)
    ...
    for item, count in sorted_item_counts:
        ...
        if cumulative_count >= 0.5 * num_transactions:
    ...
    while not association_rules:
        #print('No association rules were found with the given minimum support and confidence.')
        #print('Lowering the minimum support...')
        min_support -= 0.05
        #min_support -= 0.1
        frequent_itemsets = get_frequent_itemsets(data, min_support)        
        association_rules = get_association_rules(frequent_itemsets, min_confidence, data)
